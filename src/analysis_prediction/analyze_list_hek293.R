...
## This script analyzes the DTox-predicted HEK293 cytotoxicity scores by EPA chemical list and DrugBank approval status list
...
query_assay		<- "tox21-rt-viability-hek293-p1";
tox21_pred_file		<- "data/compound_target_probability_tox21_prediction/rt-viability-hek293-p1/tox21_compound_target_fingerprint_maccs_probability_tox21-rt-viability-hek293-p1_whole_data.tsv_rt_14_ps_5_re_0_xs_20_al_0.5_ld_0.0001_model_pred.tsv"
...
output_folder		<- "data/compound_target_probability_tox21_prediction/rt-viability-hek293-p1/dsstox_compound_target_fingerprint_maccs_probability_tox21-rt-viability-hek293-p1_whole_data.tsv_rt_14_ps_5_re_0_xs_20_al_0.5_ld_0.0001_model_pred"

## 1. Process input data file to obtain the predicted HEK293 cytotoxicity scores of Tox21 compounds
...
# read in DTox HEK293 viability model prediction result on Tox21 compounds as data frame  
...
# group the predicted HEK293 cytotoxicity scores by active/inactive compounds 
...
## 2. Analyze the predicted HEK293 cytotoxicity scores of compounds by EPA chemical list 
...
# obtain the predicted HEK293 cytotoxicity scores of compounds in each EPA list 
...
# iterate by EPA list of interest, compare the predicted HEK293 cytotoxicity scores of compounds to pos/neg controls 
dsstox_el_pred_compare <- mapply(function(eln){
	...
	# perform one-sided Mann-Whitney U test to compare predicted HEK293 cytotoxicity scores of compounds to negative controls 
	eln_neg_pv <- wilcox.test(eln_scores, tox21_neg_pred_df$predicted_outcome, alternative = "greater")$p.value;
	# perform one-sided Mann-Whitney U test to compare predicted HEK293 cytotoxicity scores of compounds to positive controls 
...
eop_lists <- setdiff(eop_lists, c("casmi2017", "tscawp", "ermodel"));
...
## 3. Analyze the predicted HEK293 cytotoxicity scores of compounds by DrugBank status list  
...
# iterate by DrugBank list of interest, compare the predicted HEK293 cytotoxicity scores of compounds to pos/neg controls  
